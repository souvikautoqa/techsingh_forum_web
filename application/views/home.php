        		...
        		<div class="row">
        			...
        			<div class="col-lg-12">
        				<div class="hero-content" data-aos="fade-down" data-aos-easing="linear">
        					...
        					<div class="vine-join flex-md-row align-items-center mb-0">
        						<a href="#" class="btn btn-md btn-mint rounded-pill">Create an Account</a>
        						<ul class="ms-3">
        							<li><a href="#"><img src="<?=base_url()?>assets/img/avatar/6.jpg" class="avatar-sm"
        										alt="image"></a></li>
										...
										?>
											...
											<div class="homesections">
												<div class="topics-left-nd-questions">
													<h2><a href="<?=base_url()?>forum/<?= $sub_category['url_slug'] ?>"> <?= $sub_category['name'] ?></a></h2>
												...
												<div class="stats">
													...
													<div>
														<?php 
															foreach($sub_category['new_posts'] as $np){
														?>
																<p><a href="<?=base_url() . $sub_category['url_slug'] . "/" . $np['url_slug'] ?>"><?= $np['title'] ?></a></p>
														<?php 
															}
														?>
													</div>
												...
												<div class="new-posts-right">
													<?php 
														...
														// 	if ($counter >= 1) break;
														if(isset($sub_category['latest_comment']) && !empty($sub_category['latest_comment'])){
													?>
															<div class="post-user">
																<div>
																	<div class="title text-color-green">
																		<a
																			href="<?= base_url() . $sub_category['url_slug'] . "/" . $sub_category['latest_comment']['url_slug']   . "
																			#comment-" . $sub_category['latest_comment']['comment_id'] ?>">
																			 <?= mb_strimwidth($sub_category['latest_comment']['comment'], 0, 50, '...') ?>
																		</a>
																	</div>
																	<div class="author-and-time-info d-flex justify-content-between">
																		<div class="author">
																			by: <?= $sub_category['latest_comment']['student_name'] ?>
																		</div>

																		<div class="topic-date">
																			<?= date('d M Y, H:i', strtotime($sub_category['latest_comment']['created_at'])) ?>
																		</div>
																	</div>
																	<div class="likes">
																		<i class="fas fa-thumbs-up"></i> <?= $sub_category['latest_comment']['total_likes_comments'] ?>
																	</div>
																</div>
															</div>
													<?php 
